...
const Modal = ({ favoriteId }) => {
  ...
  const handleOpen = () => {
    if (!open) {
      ...
      const favoriteItem = storedFavorites.find((fav) => fav.id === favoriteId);
      setFavoriteData(favoriteItem || null);
    }
    setOpen(!open);
  };
                ...
                <div className="text-center">
                  ...
                  <div className="mt-4 text-gray-600">
                    {favoriteData ? (
                      <>
                        <p>
                          <strong>Package Name:</strong> {favoriteData.name}
                        </p>
                        <p className="mt-2">
                          <strong>Reason:</strong> {favoriteData.reason}
                        </p>
                      </>
                    ) : (
